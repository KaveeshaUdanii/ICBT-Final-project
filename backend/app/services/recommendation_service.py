...
def recommend_alternative_suppliers(db: Session, supplier: Supplier, top_n: int = 2) -> list[Recommendation]:
    ...
    candidates = (
        db.execute(
            ...
            .order_by(Supplier.risk_score.asc())
            .limit(top_n)
        )
        .scalars()
        .all()
    ...
    for alt in candidates:
        ...
        text = (
            f"'{alt.name}' is recommended over '{supplier.name}' because its predicted risk score is "
            f"{risk_gap:.0f} points lower ({alt.risk_score:.0f}% vs {supplier.risk_score:.0f}%) for the same "
